...
# Astar算法的函数
def Astar(startStat,steps,answer):
...
if __name__ == '__main__':
    ...
    url = ' http://47.102.118.1:8089/api/problem?stuid=031802508'
    # form形式发送表单
    headers = {"Accept": "test", "Content-Type": "application/json"}
    data = {"id": "1001", "key1": "value1", "key2": "value2", "b": 100}
    r = requests.get(url, data=data, headers=headers,stream=True)
    r_dic = 提交.loads(r.text)
    imagecode = r_dic["img"]
    uuid=r_dic["uuid"]
    step = r_dic['step']
    swap = r_dic['swap']
    ...
    stat = image_list
    answer=""
    new_stat,steps,answer,swap0=Astar(stat,0,answer)
    #print(new_stat),可以输出
    swapmine=swap0
    new_target,steps,answer,swap0=Astar(new_stat,steps,answer)
    print(answer)
    #提交答案
